...
void solve() {
    ...
    for(int i = 2; i <= n; i++) p[i] = G[0][i-2];

    for(int i = n+1; i <= 2*n; i++) p[i] = G[n-1][i-n-1];

    vector<char> seen(2*n + 1, false);
    for(int i = 2; i <= 2*n; i++) if(p[i]) seen[p[i]] = true;

    for(int v = 1; v <= 2*n; v++) if(!seen[v]) { p[1] = v; break; }
